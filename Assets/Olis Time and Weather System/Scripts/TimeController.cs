...
namespace TimeWeather
{
    ...
    public class TimeController : MonoBehaviour
    {
        ...
        [Tooltip("Toggle whether to show the date on the UI")]
        public bool showDate;
        [Tooltip("Toggle whether to show the season on the UI")]
        public bool showSeason;
        [Tooltip("Toggle whether to show the year on the UI")]
        ...
        public void SetDayText()
        {
            if (toggleTimeUI && dayText != null)
            {
                ...
                if (showDate)
                    dayText.text += ", " + currentMonthData.month + " " + dayOfMonth;
                if (showSeason)
                dayText.text += ", \n" + currentMonthData.season;
                if (showYear)
                    dayText.text += ", " + currentYear;
        ...
        public void ProgressDays(int numToProgress = 1)
        {
            for (int n = 0; n < numToProgress; n++)
            {
                ...
                if (toggleTimeUI && dayText != null)
                {
                    if (showDate && showSeason && showYear)
                        SetDayText();
                }
            }
